...
loss=dice_coef_loss
filepath='/home/xjin/Brats_output/checkpoint-{epoch:02d}-{val_acc:.2f}.hdf5'
checkpointer=ModelCheckpoint(filepath,monitor='val_acc',verbose=1,period=5)
...
model.load_weights('/home/xjin/brats_dataset_slices/weights_11_16_3.hdf5')
model.compile(loss=loss,optimizer=opt,metrics=[dice_coef])
#K.set_value(model.optimizer.lr,K.get_value(model.optimizer.lr)*1)
model.fit_generator(generator=training_generator, validation_data=validation_generator,epochs=20,workers=4,verbose=1)

model.save_weights('/home/xjin/brats_dataset_slices/weights_11_17_20more_epochs_continue.hdf5')
