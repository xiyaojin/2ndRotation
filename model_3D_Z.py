...
from bilinear_upsampling_3D_Z import *
...
def AtrousFCN_Resnet50_16s(input_shape = None, weight_decay=0, batch_momentum=0.9, batch_shape=None, classes=21):
    ...
    bn_axis = 4
    nb_filters=[32, 32, 32]
    x = Conv3D(64, (7, 7, 7), strides=(2, 2, 2), padding='same', name='conv1', kernel_regularizer=l2(weight_decay))(img_input)
    ...
    x = conv_block3D(3, nb_filters, stage=3, block='a', weight_decay=weight_decay, strides=(2, 2, 2),batch_momentum=batch_momentum)(x)
    ...
    x = atrous_identity_block3D(3, [32, 32, 32], stage=5, block='b', weight_decay=weight_decay, atrous_rate=(2, 2, 2), batch_momentum=batch_momentum)(x)
   # x = UpSampling3D(size=(4,4,1))(x)
    x = atrous_identity_block3D(3, [32, 32, 32], stage=5, block='c', weight_decay=weight_decay, atrous_rate=(2, 2, 2), batch_momentum=batch_momentum)(x)
    #classifying layer
